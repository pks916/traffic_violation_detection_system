...
cap = cv2.VideoCapture("input/traffic2.mp4")
bike_model = YOLO('models/bike_plate_helmet.pt')
        ...
        for bxs1, bxs2 in zip(r1.boxes.data, r2.boxes.data):
            if bxs1[5] == 1:
                if curr_frame - last_overloading_frame >= 25:
                    cv2.imwrite(f'output/overloading/{uuid.uuid1()}.png', frame)
                    last_overloading_frame = curr_frame 

            if bxs2[5] == 3:
                if curr_frame - last_helmet_frame >= 25:
                    cv2.imwrite(f'output/no_helmets/{uuid.uuid1()}.png', frame)
